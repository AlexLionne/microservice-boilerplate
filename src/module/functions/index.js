    ...
    try {
        actionsState.forEach(({ name, id }) => {
            if (actionsState[id].runOnStartup) {
                if (actions.running)
                    logger.error(`Action ${name} already running, stop it to start it again`);
...
function setupActions(service) {
    ...
    if (config.actions) {
        config.actions.forEach((action, index) => {
            if (!action.cron)
                logger.warn("No periodicity provided !");
            if (!action.name)
                throw "No function provided !";
            actions.push({
                id: index,
                action: action.cron
                    ? new CronJob(action.cron, () => __awaiter(this, void 0, void 0, function* () {
                        yield handler[action.name]({
                            action,
                    ...
                    : {
                        start: () => __awaiter(this, void 0, void 0, function* () {
                            return yield handler[action.name]({
                                action,
                                waitForMessage: (topic, cb) => waitForMessage(service, topic, cb),
                                publishInternalMessage: (topic, message) => publishInternalMessage(service, topic, message),
                                publishExternalMessage: (topic, message) => publishExternalMessage(service, topic, message),
    ...
    if (messaging) {
        if (messaging.internal) {
            if (messaging.internal.events) {
                ...
                messaging.internal.events.forEach((event) => logger.info([
                    event.name,
                    event.description
                        ? "\n" + event.description
                        : "\n" + "No description provided" + "\n",
        ...
        if (messaging.external) {
            if (messaging.external.mqtt) {
                if (messaging.external.mqtt.events) {
                    ...
                    messaging.external.mqtt.events.forEach((event) => logger.info([
                        event.name,
                        event.description
                            ? "\n" + event.description
                            : "\n" + "No description provided" + "\n",
            ...
            if (messaging.external.socket) {
                if (messaging.external.socket.events) {
                    ...
                    messaging.external.socket.events.forEach((event) => logger.info([
                        event.name,
                        event.description
                            ? "\n" + event.description
                            : "\n" + "No description provided" + "\n",
...
function request(microservice, route) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            return yield http(microservice, route);
        }
        catch (e) {
            ...
            if (config.messaging) {
                ...
                if (config.messaging.internal) {
                    for (const queue of config.messaging.internal.events) {
                        yield channel.assertQueue(queue.name);
                        yield channel.consume(queue.name, (message) => {
                            const content = message.content.toString();
                            logger.info(`[${config.name}] Getting Event [${queue.name}] <- From event source`);
                            if (!handler[queue.name]) {
                                logger.error(`[${config.name}] Getting Event [${queue.name}] <- From event source`);
                            }
                            if (handler[queue.name]) {
                                handler[queue.name]({
                                    logger,
                ...
                if (config.messaging.external) {
                    if (config.messaging.external.mqtt) {
                        ...
                        if ((config.messaging.external.mqtt.events &&
                            config.messaging.external.mqtt.events.length) > 0) {
                            logger.info(`${config.messaging.external.mqtt.events.length} mqtt events to register`);
                            for (const event of config.messaging.external.mqtt.events) {
                                logger.info(`Registering event ${event.name}`);
                                client.on(event.name, (topic, message) => handler[event.name]({
                                    server: client,
                                    publishExternalMessage: (topic, message) => publishExternalMessage(service, topic, message),
                                }, topic, message));
                            }
                        ...
                        io.on("connection", (client) => {
                            ...
                            if (clientType) {
                                if (query &&
                                    ...
                                    if (connected) {
                                        logger.info(`Client ${name} not connected, update client reference (connection update)`);
                                        logger.info(["Connected clients", clients.size, clients.keys()].join(" "));
                                        connected.leave("event-room");
                                    }
                                    // update client
                                    connected.join("event-room");
                                    if ((config.messaging.external.socket.events &&
                                        config.messaging.external.socket.events.length) > 0) {
                                        logger.info(`${config.messaging.external.socket.events.length} socket events to register`);
                                        for (const event of config.messaging.external.socket.events) {
                                            logger.info(`Registering event ${event.name}`);
                                            connected.on(event.name, (data) => handler[event.name]({
                                                server: io,
                                                socket: client,
                                                publishExternalMessage: (topic, message) => publishExternalMessage(service, topic, message),
                                            }, data));
...
function publishMQTTtMessage(service, topic = "event", message = {}) {
    return __awaiter(this, void 0, void 0, function* () {
        const mqtt = service.get("mqtt");
        const config = service.get("config");
        if (!mqtt)
            return;
        const payload = Object.assign(Object.assign({}, message), { source: config.name, sentAt: new Date().toISOString() });
        mqtt.publish(topic, JSON.stringify(payload));
    });
}
function publishSocketMessage(service, topic = "event", message = {}) {
    return __awaiter(this, void 0, void 0, function* () {
...
function publishExternalMessage(service, topic = "socket://event:event", message = {}) {
    return __awaiter(this, void 0, void 0, function* () {
...
function publishInternalMessage(service, topic = "event", message = {}) {
    return __awaiter(this, void 0, void 0, function* () {
        const channel = service.get("channel");
        try {
            yield channel.assertQueue(topic);
